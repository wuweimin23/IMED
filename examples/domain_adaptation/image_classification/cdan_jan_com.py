...
def main(args: argparse.Namespace):
    ...
    # acc1 = utils.validate(test_loader, classifier_cdan,classifier_jan, head,head_true,combination,num_classes,classifier_feature_dim, W,args, device)
...
def train_student(train_source_iter, train_target_iter, classifier_cdan,classifier_jan, head, head_true,combination, student_model, student_head, optimizer_student, lr_scheduler_student, W, epoch, args):
    ...
    for i in range(args.iters_per_epoch):
        ...
        loss = acc_loss + tea_stu_loss * args.trade_off4 *tem *tem + tea_stu_loss_feature * args.trade_off3 *tem *tem
# 
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model1: ImageClassifier1, model2: ImageClassifier1,
          model_head:Head,model_head_true:Head, model_combination:Combination_Korn ,domain_adv: ConditionalDomainAdversarialLoss, domain_adv2: ConditionalDomainAdversarialLoss, optimizer_cdan: SGD,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trade-off1', default=1., type=float,
                        help='the trade-off1 hyper-parameter for transfer loss')
    parser.add_argument('--trade-off2', default=1., type=float,
                        help='the trade-off2 hyper-parameter for transfer loss')
    parser.add_argument('--trade-off3', default=1., type=float,
                        help='the trade-off3 hyper-parameter for transfer loss')
    parser.add_argument('--trade-off4', default=1., type=float,
                        help='the trade-off4 hyper-parameter for transfer loss')
    parser.add_argument('--tem', default=1., type=float,
