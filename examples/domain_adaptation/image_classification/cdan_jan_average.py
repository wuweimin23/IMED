...
def main(args: argparse.Namespace):
    ...
    for epoch in range(args.epochs):
        # train for one epoch
        train(train_source_iter, train_target_iter, classifier_cdan,classifier_jan, head,  domain_adv_cdan, optimizer_cdan,
              lr_scheduler_cdan, jmmd_loss_jan,optimizer_jan,lr_scheduler_jan, optimizer_head, lr_scheduler_head, epoch, args)

        # evaluate on validation set
        acc1 = utils.validate_avg(val_loader, classifier_cdan,classifier_jan, head,num_classes,classifier_feature_dim,args, device)
    ...
    for epoch in range(args.distill_epochs):
        train_student(train_source_iter, train_target_iter, classifier_cdan,classifier_jan, head,  student_model, student_head, optimizer_student, lr_scheduler_student, W, epoch, args)
...
def train_student(train_source_iter, train_target_iter, classifier_cdan,classifier_jan, head, student_model, student_head, optimizer_student, lr_scheduler_student, W, epoch, args):
    ...
    for i in range(args.iters_per_epoch):
        ...
        with torch.no_grad():
            ...
            y_cdan = head(f_cdan)
            y_jan = head(f_jan)
            f_com = 0.5 * (f_cdan + f_jan)
            y_com = 0.5*(y_cdan + y_jan)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model1: ImageClassifier1, model2: ImageClassifier1,
          model_head:Head,domain_adv: ConditionalDomainAdversarialLoss, optimizer_cdan: SGD,
          lr_scheduler_cdan: LambdaLR, jmmd_loss: JointMultipleKernelMaximumMeanDiscrepancy, optimizer_jan: SGD,
          lr_scheduler_jan: LambdaLR, optimizer_head: SGD, lr_scheduler_head: LambdaLR,epoch: int, args: argparse.Namespace):
