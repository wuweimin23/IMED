...
def main(args: argparse.Namespace):
    ...
    backbone = utils.get_model(args.arch, pretrain=not args.scratch)   #backbone 为resnet50, 特征提取器中再加pooling, boneneck，之后再加classifier
    ...
    print("best_acc1 = {:3.1f}".format(best_acc1))  #训练过程中的最佳目标域精确度

    # # evaluate on test set
    classifier.load_state_dict(torch.load(logger.get_checkpoint_path('best')))

    classifier.train()
    vis = GradCAM(classifier, device)
    count = 0
